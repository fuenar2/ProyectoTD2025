...
tickets_df <- do.call(rbind, lista_tickets[!sapply(lista_tickets, is.null)])
...
¿Cuáles son los 5 productos, de los vendidos por unidades, con más ventas ? ¿Cuántas unidades de cada uno se han vendido ? Si consideramos la categoría de FRUTAS Y VERDURAS. Cuáles son los 5 productos más vendidos ? ¿Cuántos kilos se han vendido de cada uno de estos productos ? Si consideramos la categoría de PESCADO. Cuáles son los 5 productos más vendidos ? ¿Cuántos kilos se han vendido de cada uno de estos productos ? Muestra mediante un gráfico de líneas como ha variado el precio por kilo de las bananas y los plátanos en los tickets disponibles, a lo largo del tiempo. ¿ Cuál es la procedencia de los tickets ?¿ Qué ciudad/ pueblo tiene un mayor número de tickets ? Muestra mediante un diagrama el número de tickets recogidos cada día de las semana. ¿Si tuvieses que cerrar un día entre semana qué día lo harías ?
...
```{r}
#Analizamos primero el nombre de todas las columnas para que el analisis sea más claro
colnames(tickets_df)



# Agrupar por producto y sumar cantidad vendida
top_5_unidades <- tickets_df %>%
  group_by(producto) %>%
...
# Gráfico
library(ggplot2)
ggplot(top_5_unidades, aes(x = reorder(producto, unidades_vendidas), y = unidades_vendidas)) +
...
Para responder esta pregunta, se ha analizado la columna producto del conjunto de datos y se ha calculado el total de unidades vendidas a través de la variable cantidad. Posteriormente, se han ordenado los productos en función del total acumulado y se han seleccionado los cinco con mayor número de unidades vendidas.

Como se muestra en la tabla, los cinco productos más vendidos por unidades han sido:
...
Se observa que los productos lácteos (leche y queso), así como productos de consumo básico como legumbres, pan y cereales, lideran el volumen de ventas por unidades. Esto puede indicar una alta frecuencia de consumo y reposición, probablemente porque son productos básicos en la cesta de la compra semanal.

Este análisis es útil para identificar productos de alta rotación, lo cual tiene aplicaciones prácticas en gestión de inventario y promociones específicas.

Nota: El gráfico representa las cantidades totales vendidas por producto de manera descendente, lo que permite una interpretación visual clara y directa de los datos.
